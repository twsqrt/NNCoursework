...
public static class TensorFactory
{
    ...
    private static ITensor Create(float[] data, Type type, TensorShape shape)
    {
    ...
    public static T Create<T>(float[] data, TensorShape shape) where T : ITensor
        => (T) Create(data, typeof(T), shape);
    
    public static T CreateZero<T>(TensorShape shape) where T : ITensor
    {
    ...
    public static T CreateRandom<T>(TensorShape shape, float min = -1.0f, float max = 1.0f) where T : ITensor
    {
        float[] data = Enumerable.Range(0, shape.Dimension)
